...
$Script = {
    ...
    if ($ScanResults) {
        ...
        ForEach ($tempfile in $TempDirectory) {
            $tempfile_ext = [IO.Path]::GetExtension($tempfile)

            If ($tempfile_ext -eq ".zip") {
                # Open ZIP archive
                $archive = [IO.Compression.ZipFile]::OpenRead($_).Entries
        ...
        <# Get-ChildItem "$TempDirectory" -Filter *.zip | ForEach-Object {
